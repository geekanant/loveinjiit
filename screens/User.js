...
reload(){
    ...
    if (user) {
      ...
      firebase.database().ref('UsersList/').orderByChild('userIndex').startAt(childData).endAt(childData+50).once('value', function(snapshot) {
  ...
  //   tasks.push(key,childData);
      ...
      if(key!=  firebase.auth().currentUser.uid&& Users.length<=50){
  ...
  render() {
    ...
    return (
      ...
      <Body>
      <Text style={{textAlign:'center',color:'#2f3542',fontSize:20,fontWeight: 'bold',fontFamily:'Cherie',
      }}>Love in JIIT</Text>
      </Body>

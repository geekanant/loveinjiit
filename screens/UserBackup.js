...
reload(){
    ...
    if (user) {
      ...
      firebase.database().ref('UsersList/').orderByChild('userIndex').startAt(childData).endAt(childData+20).once('value', function(snapshot) {
  ...
  //   console.log(childData);
  //   tasks.push(key,childData);

      console.log(Users.length)
      if(key!=  firebase.auth().currentUser.uid&& Users.length<=20){
  ...
  render() {
    ...
    return (
      ...
      <Body>
      <Text style={{textAlign:'center',color:'#2f3542',fontSize:20,fontWeight: 'bold',fontFamily:'Avenir-Medium',
      }}>Love in JIIT</Text>
      </Body>
      ...
      <Text style={{textAlign:'center',justifyContent:'center',alignItems:'center',fontFamily:'Avenir-Medium'}}> {"Tap till Users load\n Or wait for more users to Sign Up"}</Text>
      </View>
          {this.renderUsers()}
    </View>
  </View>
